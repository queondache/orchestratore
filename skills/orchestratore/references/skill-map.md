...
|---|---|---|---|
| Architettura, DB, tier, sicurezza | `senior-architect` + skill di progetto (`mesa-senior-architect`, `pau-senior-architect`, `zerocampo-senior-architect`) | nativa | per path: `~/Dev/skills/<nome>/SKILL.md` oppure `~/.claude/skills/<nome>/SKILL.md` — le skill di progetto stanno in `~/.claude/skills/`, quelle di metodo in `~/Dev/skills/`; risolvi il path prima di passarlo |
...
| Verifica | agent `verificatore` del plugin + [verifica](verifica.md) | nativo | opus via bridge, o thread `gpt-5.6-sol` con il testo dell'agent per path |
| Integrazione branch task | agent `integratore` del plugin | nativo | thread `gpt-5.6-terra` |
