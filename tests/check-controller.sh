...
need "$CTL" --db "$DB" add-task run-a task-a 'persistent flow' --builders 2 --conflict-key task-a
...
need "$CTL" --db "$DB" add-task run-a task-b rollover --conflict-key task-b
...
need "$CTL" --db "$DB" add-task run-a task-d aba-dedup --conflict-key task-d
...
need "$CTL" --db "$DB" add-task run-a task-e recovery-freeze --conflict-key task-e
...
need "$CTL" --db "$DB" add-task run-a task-c partial-finalize --conflict-key task-c
...
need "$CTL" --db "$DB" add-task run-config task-config configured --conflict-key task-config
...
need "$CTL" --db "$DB" add-task run-config task-config-2 configured-2 --conflict-key task-config-2
...
# Six ready builders never dispatch more than five milestone slots.
need "$CTL" --db "$DB" start run-builders
need "$CTL" --db "$DB" acquire run-builders owner-a --seconds 60
for n in 1 2 3 4 5 6; do need "$CTL" --db "$DB" add-task run-builders "builder-$n" builder --conflict-key "builder-$n"; done
for n in 1 2 3 4 5; do need "$CTL" --db "$DB" complete "builder-$n" --evidence strategy; done
if "$CTL" --db "$DB" complete builder-6 --evidence strategy --holder owner-a >/dev/null 2>&1; then
  echo "sixth builder exceeded milestone cap" >&2; exit 1
fi
OUT="$($CTL --db "$DB" schedule run-builders --dry-run)"
[[ "$(rg -c '"action": "build"' <<<"$OUT")" -eq 5 ]]
...
if "$CTL" --db "$DB" add-task run-lease task-nolease solo --conflict-key task-nolease >/dev/null 2>&1; then
  echo "add-task accepted without --holder" >&2; exit 1
fi
if "$CTL" --db "$DB" add-task run-lease task-wrong solo --conflict-key task-wrong --holder owner-x >/dev/null 2>&1; then
  echo "add-task accepted a holder without a valid lease" >&2; exit 1
fi
need "$CTL" --db "$DB" add-task run-lease task-lease solo --conflict-key task-lease

need "$CTL" --db "$DB" start run-expired --holder owner-a --seconds -1
if "$CTL" --db "$DB" add-task run-expired task-expired solo --conflict-key task-expired --holder owner-a >/dev/null 2>&1; then
  echo "add-task accepted an expired lease" >&2; exit 1
fi
need "$CTL" --db "$DB" acquire run-expired owner-b --seconds 60
"$CTL" --db "$DB" add-task run-expired task-recovered solo --conflict-key task-recovered --holder owner-b >/dev/null
